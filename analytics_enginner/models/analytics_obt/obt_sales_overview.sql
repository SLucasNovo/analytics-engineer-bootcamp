...
dp.product_id           as products_product_id,
dp.product_code         as products_product_code,
dp.product_name         as products_product_name,
...
fs.order_id             as sales_order_id,
--fs.product_id           as sales_product_id,
--fs.customer_id          as sales_customer_id,
--fs.employee_id          as sales_employee_id,
fs.shipper_id           as sales_shipper_id,
fs.quantity             as sales_quantity,
fs.unit_price           as sales_unit_price,
fs.discount             as sales_discount,
fs.status_id            as sales_status_id,
fs.date_allocated       as sales_date_allocated,
fs.purchase_order_id    as sales_purchase_order_id,
fs.inventory_id         as sales_inventory_id,
fs.order_date           as sales_order_date,
fs.shipped_date         as sales_shipped_date,
fs.paid_date            as sales_paid_date,
current_timestamp() as insertion_timestamp


from {{ ref ('fact_sales')}} fs 
left join {{ ref ('dim_customer')}} dc on dc.customer_id = fs.customer_id
left join {{ ref ('dim_employee')}} de on de.employee_id = fs.employee_id
left join {{ ref ('dim_products')}} dp on dp.product_id = fs.product_id
